...
def query_graphql():
    ...
    try:
        ...
        for rank, (wallet, count) in enumerate(ranking, start=1):
            table_data.append([rank, wallet, count])

        # Crear el widget de salida
        output = widgets.Output()

        # Crear el widget de la tabla
        table = widgets.HTML(
            value='<table><tr><th>RANK</th><th>ADDRESS</th><th>ATTESTATIONS</th></tr></table>'
        )

        # Función para actualizar la tabla
        def update_table():
            table.value = '<table><tr><th>RANK</th><th>ADDRESS</th><th>ATTESTATIONS</th></tr>'
            for row in table_data:
                table.value += f'<tr><td>{row[0]}</td><td>{row[1]}</td><td>{row[2]}</td></tr>'
            table.value += '</table>'
